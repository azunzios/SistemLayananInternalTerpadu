...
import type { User, Ticket, Category, SparepartRequest, AuditLog, Notification, WorkOrder, KartuKendali, KartuKendaliEntry } from '../types';
import { api } from './api';
...
const datasetLoaded: Record<'tickets' | 'categories' | 'users' | 'workOrders' | 'notifications', boolean> = {
  tickets: false,
  categories: false,
  users: false,
  workOrders: false,
...
export async function loadTicketsPage(page: number = 1, opts?: { type?: string }) {
  ...
  try {
    ...
    ticketsMeta.perPage = meta?.per_page ?? res?.per_page ?? ticketsMeta.perPage;
...
async function loadWorkOrdersFromApi(force = false): Promise<WorkOrder[]> {
  ...
  try {
    const res = await api.get<any>('work-orders?per_page=100').catch(() => ({}));
...
async function loadNotificationsFromApi(force = false): Promise<Notification[]> {
  if (!force && datasetLoaded.notifications && cache.notifications.length > 0) {
    return cache.notifications;
  }
  try {
    const res = await api.get<any>('notifications?per_page=100').catch(() => ({}));
...
const roleDatasetMap: Record<string, (keyof typeof datasetLoaded)[]> = {
  ...
  super_admin: ['tickets', 'users', 'categories', 'workOrders', 'notifications'],
};
...
export async function loadDataFromApiOnce(role: string = 'pegawai') {
  ...
  const tasks = datasets
    ...
    .map(dataset => datasetLoaders[dataset]().catch(err => {
...
export async function refreshTicketsFromApi() {
  ...
  try {
    ...
    await loadTicketsPage(1);
    console.log('✅ Tickets first page refreshed from API. Total:', ticketsMeta.total);
  } catch (err) {
...
export async function loadAllZoomMeetingTickets() {
  ...
  let page = 1;
  while (true) {
    const res: any = await api.get<any>(`tickets?type=zoom_meeting&per_page=100&page=${page}`).catch(() => ({}));
    const data = Array.isArray(res) ? res : (res?.data || []);
    all.push(...data);
...
export async function getTicketCounts(type?: string): Promise<{ total: number; pending: number; in_progress: number; completed: number; rejected: number; }> {
  try {
...
export const loginUser = async (email: string, password: string): Promise<{ access_token: string; token_type: string; user: User }> => {
  try {
    const response = await api.post<{ access_token: string; token_type: string; user: any }>('login', {
      email,
      password,
    });
    ...
    const roles: string[] = Array.isArray(raw.roles) ? raw.roles : (raw.role ? [raw.role] : ['pegawai']);
    console.log(raw);
    const normalizedUser: User = {
      ...
      failedLoginAttempts: Number(raw.failedLoginAttempts ?? raw.failed_login_attempts ?? 0),
      lockedUntil: raw.lockedUntil ?? raw.locked_until ?? undefined,
    };

    return { access_token: response.access_token, token_type: response.token_type, user: normalizedUser };
  } catch (err) {
...
export const createWorkOrder = async (data: Partial<WorkOrder> & { ticketId: string; type: 'sparepart' | 'vendor'; createdBy: string }) => {
...
export const createOrUpdateKartuKendali = async (assetCode: string, assetNUP: string, assetName: string, entry: KartuKendaliEntry) => {
  const kartuList = getKartuKendali();
 ...
 */
export async function getTicketComments(ticketId: string, page: number = 1, perPage: number = 30): Promise<CommentsResponse> {
  try {
    const response = await api.get<CommentsResponse>(`/tickets/${ticketId}/comments?page=${page}&per_page=${perPage}`);
    return response;
  } catch (error) {
 ...
 */
export async function createTicketComment(ticketId: string, content: string, parentCommentId?: number): Promise<Comment> {
  try {
