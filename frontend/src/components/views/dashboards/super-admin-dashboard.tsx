...
export const SuperAdminDashboard: React.FC<SuperAdminDashboardProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // Fetch tickets (semua page untuk dashboard stats)
        const ticketsRes: any = await api.get("tickets?per_page=1000");
        const ticketsData = Array.isArray(ticketsRes)
          ? ticketsRes
          : ticketsRes?.data || [];
        setTickets(ticketsData);

        // Fetch users
        const usersRes: any = await api.get("users?per_page=1000");
    ...
    return {
      ...
      pendingTickets: tickets.filter((t) =>
        [
          "menunggu_review",
          "pending_approval",
          "menunggu_verifikasi_penyedia",
        ].includes(t.status)
      ).length,
      completedTickets: tickets.filter((t) => t.status === "closed").length,
      rejectedTickets: tickets.filter((t) =>
        ["ditolak", "rejected"].includes(t.status)
      ).length,
      ticketsLast7Days: tickets.filter(
  ...
  const usersByRole = useMemo(() => {
    return [
      ...
      {
        name: "Admin Layanan",
        value: users.filter((u) => u.role === "admin_layanan").length,
      },
      {
        name: "Admin Penyedia",
        value: users.filter((u) => u.role === "admin_penyedia").length,
      },
      {
        name: "Teknisi",
        value: users.filter((u) => u.role === "teknisi").length,
      },
      {
        name: "Pegawai",
        ...
        <div className="flex items-center justify-center gap-6 px-8 md:px-40 mt-10">
          <motion.button
            ...
            className="
              ...
              border-t border-white/40 border-b border-blue-500/20
          ...
          >
            ...
            <div className="relative flex items-center justify-between gap-4 z-10">
              <div className="text-left pl-2">
                <p className="text-blue-100 text-xs font-medium uppercase tracking-wider drop-shadow-sm">
                  User Management
                </p>
              ...
              <div className="h-10 w-10 bg-blue-700 rounded-full flex items-center justify-center shadow-inner border border-white/10">
