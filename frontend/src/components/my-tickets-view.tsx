...
import { AlertCircle, Search, RotateCcw, Eye, Calendar, Clock, Wrench, Video, Loader } from 'lucide-react';
...
export const MyTicketsView: React.FC<MyTicketsViewProps> = ({ onViewTicket }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterType, setFilterType] = useState<string>('all');
  const [selectedTab, setSelectedTab] = useState<'all' | 'pending' | 'inProgress' | 'completed'>('all');
  const [tickets, setTickets] = useState<Ticket[]>([]);
  const [pagination, setPagination] = useState<PaginationMeta | null>(null);
  const [stats, setStats] = useState<TicketStats>({ total: 0, pending: 0, in_progress: 0, completed: 0, rejected: 0 });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Pantau semua tiket yang Anda ajukan</p>
        </div>
      </div>
          ...
          <Tabs value={selectedTab} onValueChange={(val) => setSelectedTab(val as any)} className="w-full">
            <TabsList className="grid grid-cols-4 w-full">
              ...
              <TabsTrigger value="inProgress">
                Diproses {statsLoading ? <Loader className="h-3 w-3 animate-spin ml-1" /> : `(${stats.in_progress})`}
              </TabsTrigger>
              <TabsTrigger value="completed">
                Selesai {statsLoading ? <Loader className="h-3 w-3 animate-spin ml-1" /> : `(${stats.completed})`}
              </TabsTrigger>
            </TabsList>

            {(['all', 'pending', 'inProgress', 'completed'] as const).map((tab) => (
                ...
                {loading ? (
                  ...
                  <Card>
                    <CardContent className="flex flex-col items-center justify-center py-16">
                      ...
                      <p className="text-muted-foreground text-center">
                        ...
                        {tab === 'inProgress' && 'Tidak ada tiket dalam proses'}
                  ...
                  <div className="space-y-4">
                    <div className="grid gap-4">
                      {tickets.map((ticket) => (
                        <Card key={ticket.id} className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => onViewTicket(ticket.id)}>
                          <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="flex items-center gap-2">
                                    {React.createElement(getTypeIcon(ticket.type), {
                                      className: `h-5 w-5 ${
                                        ticket.type === 'perbaikan' ? 'text-orange-600' : 'text-purple-600'
                                      }`
                                    })}
                      ...
                      <Card>
                        <CardContent className="flex items-center justify-between py-4">
                          <div className="text-sm text-muted-foreground">
                            Halaman {pagination.current_page} dari {pagination.last_page} • Menampilkan {pagination.from}-{pagination.to} dari {pagination.total}
                          </div>
